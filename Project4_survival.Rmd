...
## 2) Kaplan-Meier curves (5 баллов)
...
### 3) Groups that differ in survival (5 баллов)

For age group "AG" p-valueis equal to 0.002903, then thease group are different.
```{r}
logrank_test(Surv(futime, fustat) ~ AG, data = ovar)
```

For resid.ds p-valueis equal to 0.05444, then thease group are almost different.
```{r}
...
### 4) Estimations of factors that affect the risk and risk ratio (7 баллов)
